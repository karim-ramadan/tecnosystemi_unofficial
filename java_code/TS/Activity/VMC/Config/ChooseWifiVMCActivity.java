...
public class ChooseWifiVMCActivity extends BaseActivity {
    ...
    public void onCreate(Bundle bundle) {
        setContentView(R.layout.activity_choose_wifi_vmcactivity);
        this.activity = this;
        this.typeActStyle = 3;
    ...
    public void aggiornaWifi() {
        ...
        if (Constants.ISDEMO) {
            ...
            this.lvWifi.setAdapter(new WifiAdapter(this, this.wifis, Constants.DEVICE_TYPE_VMC));
            return;
        }
        try {
                ...
                public void run() {
                    new Thread(new Runnable() {
                        public void run() {
                            ...
                            MySocket.initInstance(ChooseWifiVMCActivity.this.activity, ChooseWifiVMCActivity.this.activity, false);
                            ChooseWifiVMCActivity.this.showProgress();
                            CmdPICO cmdPICO = new CmdPICO();
                            cmdPICO.setPin(ChooseWifiVMCActivity.this.pin);
                            cmdPICO.setCmd(Protocols.CMD_GET_WIFI);
                            UDPSocket.startListening();
                            final String sendCMD = UDPSocket.sendCMD(cmdPICO, 10000, 1500);
                            ChooseWifiVMCActivity.this.hideProgress();
                            ChooseWifiVMCActivity.this.activity.runOnUiThread(new Runnable() {
                                public void run() {
                                    ChooseWifiVMCActivity.this.addwifi(sendCMD);
                                }
                            });
                        }
                    }).start();
    ...
    public void protectedwifi(final WiFi wiFi, boolean z) {
        if (Constants.ISDEMO) {
            Intent intent = new Intent(this.activity, ConfigVMCActivity.class);
            ...
            Intent intent2 = new Intent(this.activity, ConfigVMCActivity.class);
            ...
            arrayList2.add(new TextView.OnEditorActionListener() {
                public boolean onEditorAction(TextView textView, int i, KeyEvent keyEvent) {
                    ...
                    Intent intent = new Intent(ChooseWifiVMCActivity.this.activity, ConfigVMCActivity.class);
                    wiFi.setPwd(textView.getText().toString());
                    intent.putExtra(Constants.INTENT_WIFI, wiFi);
                    intent.putExtra("name", ChooseWifiVMCActivity.this.name);
                    intent.putExtra(Constants.INTENT_SETUPMODE, ChooseWifiVMCActivity.this.mode);
                    intent.putExtra(Constants.INTENT_PIN, ChooseWifiVMCActivity.this.pin);
                    ChooseWifiVMCActivity.this.activity.startActivity(intent);
                    ChooseWifiVMCActivity.this.dismissdialog();
                    return false;
                }
            });
